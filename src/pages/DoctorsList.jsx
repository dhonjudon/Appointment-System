...
function DoctorsList() {
  ...
  useEffect(() => {
    axios
      ...
      .then((response) => {
        ...
        const specNames = specs.map(
          (spec) => spec.name || spec.specialization_name || spec,
        );
        setSpecialties(["All Specialties", ...specNames]);
      })
      .catch((error) => {
  ...
  //     .then((response) => {
  //       const hosps = response.data.data || response.data || [];
  //       const hospNames = hosps.map(
  //         (hosp) => hosp.name || hosp.hospital || hosp,
  //       );
  //       setHospitals(["All Hospitals", ...hospNames]);
  //     })
  //     .catch((error) => {
  ...
  // Fetch doctors from API (all doctors, not paginated)
  useEffect(() => {
    ...
    axios
      .get(`http://localhost:3000/api/doctors?${params.toString()}`)
      .then((response) => {
  ...
  }, [searchQuery, selectedSpecialty, selectedHospital, showAvailableOnly]);
