...
function DoctorsList() {
  ...
  const fetchDoctors = async () => {
    ...
    try {
      ...
      if (data.status === "success") {
        setDoctorsData(data.data.items || data.data); // Adjust based on your API response structure
      } else {
        setDoctorsData([]);
      }
    } catch (error) {
  ...
  // Refetch doctors when filters/search change
  useEffect(() => {
    fetchDoctors();
            ...
            {loading ? (
              ...
              doctorsData.map((doctor) => (
                <div
                  ...
                  className="group bg-white rounded-2xl border border-gray-100 overflow-hidden hover:border-emerald-200 hover:shadow-lg hover:shadow-emerald-50 transition-all duration-300 cursor-pointer"
                >
                  {/* ...rest of your existing doctor card UI remains unchanged */}
                </div>
              ))
            ) : (
