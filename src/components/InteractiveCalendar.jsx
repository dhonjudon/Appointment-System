...
const InteractiveCalendar = ({
  ...
  return (
    <div
      className={`bg-white rounded-[1.2rem] ${isModal ? "shadow-[0_8px_30px_rgba(0,0,0,0.12)] border border-gray-100 p-5 w-75" : "shadow-[0_12px_32px_rgba(43,136,113,0.15)] border border-gray-100 p-6 w-full"}`}
    >
      {/* Calendar Header */}
      <div
        className={`flex justify-between items-center ${isModal ? "mb-4" : "mb-6"}`}
      >
        <h3
          className={`${isModal ? "text-[1.15rem]" : "text-[1.4rem]"} font-extrabold text-gray-950 px-2 tracking-tight`}
        >
        ...
        <div className="flex space-x-2 pr-2">
          <button
            ...
            className="text-gray-400 hover:text-gray-800 transition p-2"
          >
            <svg
          ...
          <button
            ...
            className="text-gray-800 hover:text-gray-900 transition p-2"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              className="h-3.5 w-3.5"
      ...
      <div className="grid grid-cols-7 mb-4">
        {["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"].map((day) => (
          <div
            key={day}
            className={`text-center font-extrabold text-gray-800 pb-2 ${isModal ? "text-[11.5px]" : "text-[12.5px]"}`}
          >
            {day}
          </div>
        ))}
      ...
      <div className="rounded-md border-t border-l border-gray-200 overflow-hidden box-border bg-white">
        {grid.map((row, rowIndex) => (
          <div
            key={rowIndex}
            className="grid grid-cols-7 border-b border-gray-200 box-border"
          >
            {row.map((day, colIndex) => {
              ...
              const cellKey = `${year}-${String(month + 1).padStart(2, "0")}-${String(day.d).padStart(2, "0")}`;
              ...
              if (day.isPrev || day.isNext) {
                ...
                bgClass = "bg-emerald-50 border border-emerald-300";
                textClass = "text-emerald-700";
                fontClass = "font-extrabold text-[13px]";
              } else if (isHighlighted && day.isCurrent) {
                bgClass = "bg-[#e8f7ef] border border-emerald-200";
                textClass = "text-emerald-700";
                fontClass = "font-extrabold text-[13px]";
              }
              ...
              return (
                <div
                  ...
                  className={`flex items-center justify-center ${isModal ? "p-3 text-[12px]" : "p-4.5 text-[13px]"} border-r border-gray-200 box-border transition-colors ${bgClass}`}
                >
                  <span className={`relative ${textClass} ${fontClass}`}>
                    {day.d}
                    {isHighlighted && !isSelected && day.isCurrent && (
