...
const getPaymentQuote = (baseAmount, paymentMethod = "card") => {
  ...
  const platformFeeRate =
    normalizedMethod === "card" ? 0.025 : normalizedMethod === "upi" ? 0.01 : 0;
...
const doctorService = {
  ...
  // ── DOCTOR SITE: Get all schedules for a doctor (for editing) ──
...
const paymentService = {
  createPayment: async ({
    ...
    try {
      ...
      const appointment = appointmentResult.rows[0];
      if (
        !["pending", "rescheduled", "confirmed"].includes(appointment.status)
      ) {
...
const doctorController = {
  ...
  getDoctorByUserId: asyncHandler(async (req, res) => {
    ...
    if (!doctor) {
      return sendError(res, 404, "Doctor not found for this user");
    }
  ...
  getDoctorSchedules: asyncHandler(async (req, res) => {
    const doctorId = normalizeNumericId(req.params.doctorId);
    if (!doctorId) {
      return sendError(res, 400, "Valid doctorId is required");
    }
    ...
    sendSuccess(res, 200, "Doctor schedules fetched successfully", {
      items: schedules,
    });
  }),
...
const paymentController = {
  createPayment: asyncHandler(async (req, res) => {
    ...
    if (!payload.appointment_id || !payload.user_id || !payload.provider) {
      return sendError(
        res,
        400,
